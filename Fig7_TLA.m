...
UAV_numbers = 1:6; % Số lượng UAV từ 1 đến 6
results = zeros(length(UAV_numbers), 1);

for UAV_idx = 1:length(UAV_numbers)
    num_UAVs = UAV_numbers(UAV_idx); % Số lượng DBS thay đổi
    
    % Khởi tạo UAV với số lượng thay đổi
    UAV_pos = [[500, 500]; [1500, 500]; [1500, 1500]; [500, 1500]; 
              [1000, 1000]; [2000, 2000]]; % Thêm vị trí nếu cần
    UAV_pos = UAV_pos(1:num_UAVs,:);
...
MU_density = 9; % average MUs per location
...
MU_rate = rand(1, num_MUs) .* 40e6 + 1e6; 
...
% grid on;
% total_MU_rate = sum([MU([MU.UAV] > 0).rate]);
% results(UAV_idx) = total_MU_rate;
num_MU_served = sum([MU.UAV] > 0);
results(UAV_idx) = num_MU_served / num_MUs * 100;
end


figure;
plot(UAV_numbers, results, 'r-', 'LineWidth', 2);
xlabel('Number of UAVs in emergency zone', 'FontSize', 12);
ylabel('Total data rate(Mbps)', 'FontSize', 12);
% title('Fraction of served MUs data rate requirements', 'FontSize', 14);
grid on;
xticks(UAV_numbers);
xlim([min(UAV_numbers), max(UAV_numbers)]);
ylim([0 100]);
% xlim([min(UAV_numbers)-0.5 max(DBS_numbers)+0.5]);
% ylim([0 ceil(max(results/1e6)/100)*100]);
% xticks(DBS_numbers);
