...
L_total = 2000; % The total distance
...
divergence_angle = 2*10^-3; % the divergence angle
...
P_t_dBm = 25:0.5:35;    % the transmit power in dBm
...
%     R_i(i) = @(SNR) log2(1 + SNR).* integral(fun_SNR, 0, 10^(0/10));
%       integrand = @(SNR) log2(1 + SNR) .* fun_SNR(SNR);
%     R_i(i) = B_i * integral(@(SNR) log2(1 + SNR) .* fun_SNR(SNR), 0, inf);
R_i(i) = B_i * integral(@(SNR) log2(1 + SNR) .* fun_SNR(SNR), 0, 100);
