...
MU_pos = zeros(100*100*1, 2);
num_MUs = 0; 

MU_center = zeros(100*100, 2);
x_axis = -10;
for i = 1:100
    x_axis = x_axis + 20;
    y_axis = -10;
    for j = 1:100
        y_axis = y_axis + 20;
        MU_center(100*(i-1) + j, :) = [x_axis, y_axis];  
...
MU_rate = rand(1, num_MUs) .* 4e6 + 1e6; 
...
%             Access Link: required bandwidth
        ...
        for i = 1:length(MU_covered)
            MU_covered(i).bandwidth = MU_bandwidth(i); %MU_bandwidth la bang thong yeu cau
        end
...
% % Tính tổng số người dùng được phục vụ
% total_served_users = 0;
% 
% for UAV_idx = 1:num_UAVs
%     total_served_users = total_served_users + length(UAV(UAV_idx).MU_id);
% end
% 
% % disp(['Tổng số người dùng được phục vụ: ', num2str(total_served_users)]);
% 
...
% MU_served_percentage = (total_served_users / num_MUs) * 100;
% 
% % Display results
