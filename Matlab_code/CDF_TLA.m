...
BS_pos = [2500,1000]; %OIRS pos

UAV = struct;
for i = 1:num_UAVs
    UAV(i).id = i;
    UAV(i).pos = UAV_pos(i, :); % position of the UAV
    UAV(i).BS = BS_pos; % position of the BS associated with the UAV
...
MU_rate = rand(1, num_MUs) .* 50e6 + 1e6; 
...
%         [~, Index] = sort([MU_covered.rate]); %RESCUE
%         MU_sorted = MU_covered(Index);
          [~, Index] = sort([MU_covered.pathloss]); % Sắp xếp theo suy hao từ thấp đến cao(TLA)
          MU_sorted = MU_covered(Index);
...
% % % Calculate percentage of MU served and total data rate
% 
% num_MU_served = sum([MU.UAV] > 0);   % Number of MUs associated with a UAV
% total_MU_rate = sum([MU([MU.UAV] > 0).rate]);   % Total rate of MUs being served
% 
% % Percentage of MUs served
% MU_served_percentage = (num_MU_served / num_MUs)* 1000 ;
% 
% % Display results
% disp(['Percentage of MUs served: ', num2str(MU_served_percentage), '%']);
% disp(['Total data rate of MUs served: ', num2str(total_MU_rate / 1e6), ' Mbps']);
% 
% % Plotting the results
% figure;
% 
% % Bar chart for MU served percentage
% subplot(1, 2, 1); % Create a subplot for the first chart
% bar(MU_served_percentage, 'FaceColor', [0.2 0.6 0.5]);
% title('Percentage of MUs Served');
% ylabel('Percentage (%)');
% xlabel('MUs Served');
% ylim([0 100]);  % Set y-axis limits from 0 to 100
% grid on;
% 
% % Bar chart for total data rate of MUs served
% subplot(1, 2, 2); % Create a subplot for the second chart
% bar(total_MU_rate / 1e6, 'FaceColor', [0.6 0.2 0.8]); % Convert to Mbps
% title('Total Data Rate of MUs Served');
% ylabel('Data Rate (Mbps)');
% xlabel('Total Data Rate');
% grid on;
% 
...
% 
% % Percentage of MUs served
% MU_served_percentage = (num_MU_served / num_MUs) * 100; % Adjusted percentage
% 
% % Display results
% disp(['Percentage of MUs served: ', num2str(MU_served_percentage), '%']);
% disp(['Total data rate of MUs served: ', num2str(total_MU_rate / 1e6), ' Mbps']);
...
% figure;
% 
% % Plot bar chart for MU served percentage on the left axis
% yyaxis left;
% bar(1, MU_served_percentage, 0.4, 'FaceColor', [0.2 0.6 0.5]); % Bar for percentage
% ylabel('Percentage of MUs Served (%)');
% ylim([0 100]);  % Set y-axis limits for percentage
% hold on;
% 
% % Plot bar chart for total data rate on the right axis
% yyaxis right;
% bar(1.3, total_MU_rate / 1e6, 0.4, 'FaceColor', [0.6 0.2 0.8]); % Bar for data rate in Mbps
% ylabel('Total Data Rate (Mbps)');
% 
% % General plot settings
% xticks([1 1.3]); % Set x-axis tick positions
% xticklabels({'MU Served ', 'Total Data Rate'}); % Set x-axis labels
% title('Fraction of served MUs and total data rate incurred by RESCUE method');
% grid on;
% 
% 

% Tính toán CDF của các MU được phục vụ
served_MUs = MU([MU.UAV] > 0); % Lấy danh sách MU được phục vụ
if ~isempty(served_MUs)
    served_rates = [served_MUs.rate] / 1e6; % Chuyển tốc độ sang Mbps
    
    % Tính CDF thực nghiệm
    [f, x] = ecdf(served_rates); 
    
    % Vẽ biểu đồ CDF (như Hình 5)
    figure;
    plot(x, f, 'r-', 'LineWidth', 2); 
    grid on;
    xlabel('Data Rate Requirement (Mbps)', 'FontSize', 12);
    ylabel('Cumulative Fraction of Served MUs', 'FontSize', 12);
    title('CDF of Served MUs Data Rates', 'FontSize', 14);
    xlim([min(served_rates) max(served_rates)]);
end
